...
use reth_provider::{
    ...
    ChainSplitTarget, DisplayBlocksChain, ForkChoiceNotificationSender, ForkChoiceNotifications,
...
pub struct BlockchainTree<DB, E> {
    ...
    fork_choice_notification_sender: ForkChoiceNotificationSender,
...
impl<DB, E> BlockchainTree<DB, E> {
    ...
    pub fn subscribe_fork_choice(&self) -> ForkChoiceNotifications {
        self.fork_choice_notification_sender.subscribe()
    }

    /// Returns a clone of the sender for the fork choice notifications.
    pub fn fork_choice_notification_sender(&self) -> ForkChoiceNotificationSender {
        self.fork_choice_notification_sender.clone()
    }
}
    ...
    E: BlockExecutorProvider,
{
    ...
    pub fn new(
        ...
        let (fork_choice_notification_sender, _receiver) =
            tokio::sync::broadcast::channel(max_reorg_depth * 2);
    ...
    /// Replaces the fork choice notification sender.
    ///
    /// Caution: this will close any existing subscriptions to the previous sender.
    #[doc(hidden)]
    pub fn with_fork_choice_notification_sender(
        mut self,
        fork_choice_notification_sender: ForkChoiceNotificationSender,
    ) -> Self {
        self.fork_choice_notification_sender = fork_choice_notification_sender;
        self
    }
